...
export async function GET(request: Request) {
  try {
    ...
    const data = await fetchCashflow(userId, currency, period, customStart, customEnd, categoryNames)
    return ok(data)
  } catch (err) {
    logger.error('widgets-cashflow', 'GET error', { message: err instanceof Error ? err.message : String(err) })
