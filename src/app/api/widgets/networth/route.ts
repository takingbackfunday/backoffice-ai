...
export async function fetchNetWorth(
  ...
  if (names.length > 0) {
    const hasUncategorized = names.includes('Uncategorized')
    const namedCategories = names.filter((n) => n !== 'Uncategorized')
...
export async function GET(request: Request) {
  try {
    ...
    const data = await fetchNetWorth(userId, currency, period, customStart, customEnd, categoryNames)
    return ok(data)
  } catch (err) {
    logger.error('widgets-networth', 'GET error', { message: err instanceof Error ? err.message : String(err) })
