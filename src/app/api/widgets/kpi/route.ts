...
export async function fetchKpi(userId: string, currency: DashboardCurrency): Promise<KpiData> {
  const now = new Date()
...
export async function GET(request: Request) {
  try {
    ...
    const result = await fetchKpi(userId, currency)
    return ok(result)
  } catch (err) {
    logger.error('widgets-kpi', 'GET error', { message: err instanceof Error ? err.message : String(err) })
