...
- Check the payees list (from get_payees) first — if the payee already exists there, use the exact same spelling

PROJECT ASSIGNMENT:
- workspaceName MUST be copied VERBATIM from the projects list (from get_project_transactions) — do not invent or abbreviate
...
- payeeName equals is SECONDARY — only add it if there is already a payee in the payees list.
...
- SKIP any merchant that appears in the existing rules (from get_rules) — a rule already covers it
...
Focus first on patterns from the last 18 months (since ${recentCutoff.toISOString().slice(0, 10)}).`
  ...
  // LLM fetches data lazily via tools — only the snapshot goes in the initial message
