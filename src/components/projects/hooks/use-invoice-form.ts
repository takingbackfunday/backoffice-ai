    ...
    try {
      ...
      if (mode === 'create') {
        const res = await fetch(`/api/projects/${projectId}/invoices`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jobId: state.jobId || undefined, dueDate: state.dueDate, currency: state.currency, notes: state.notes || undefined, lineItems: lineItemsPayload, quoteId: props.quoteId || undefined }),
        })
        const json = await res.json()
        ...
        const res = await fetch(`/api/projects/${projectId}/invoices/${existingInvoiceId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ jobId: state.jobId || undefined, dueDate: state.dueDate, currency: state.currency, notes: state.notes || undefined, lineItems: lineItemsPayload }),
        })
        const json = await res.json()
