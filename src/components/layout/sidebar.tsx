  ...
  return (
    ...
    >
      ...
      <ul className="flex flex-col gap-0.5">
        ...
        {/* Collapse toggle — styled as a section header */}
        <li>
          <button
            onClick={toggleCollapsed}
            className={cn(
              'w-full flex items-center transition-colors text-[11px] font-semibold uppercase tracking-wide bg-[#2a2a2a] text-[#e5e5e5] hover:bg-[#333333]',
              collapsed ? 'justify-center px-2 py-1.5' : 'gap-2.5 px-4 py-1.5'
            )}
            aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
            title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
          >
            <svg
              className={cn('w-3.5 h-3.5 shrink-0 transition-transform duration-200', collapsed ? 'rotate-180' : '')}
              fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"
            >
