...
export function BusinessTypePicker({ counts }: Props) {
  ...
  const [redirectTarget, setRedirectTarget] = useState('/bank-accounts')
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // Store businessType in localStorage for sidebar
      localStorage.setItem('businessType', selected)

      const target = json.data.redirectTo ?? '/bank-accounts?onboarding=1'
      setRedirectTarget(target)

      if (json.data.overheadWorkspace) {
